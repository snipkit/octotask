...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          octotask_user_profile: this._safeGetItem('octotask_user_profile'),
          octotask_settings: this._safeGetItem('octotask_settings'),
          octotask_profile: this._safeGetItem('octotask_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('octotask_viewed_features'),
          developer_mode: this._safeGetItem('octotask_developer_mode'),
        ...
        ui: {
          // Tab configuration
          octotask_tab_configuration: this._safeGetItem('octotask_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('octotask_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('octotask_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          octotask_read_logs: this._safeGetItem('octotask_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('octotask_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('octotask_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.octotask_tab_configuration) {
        try {
          this._safeSetItem('octotask_tab_configuration', data.ui.octotask_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'octotask_acknowledged_debug_issues',
        'octotask_acknowledged_connection_issue',
        'error_logs',
        'octotask_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('octotask_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
