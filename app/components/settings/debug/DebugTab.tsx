...
const GITHUB_URLS = {
  original: 'https://api.github.com/repos/octotask/octotask/commits/main',
  fork: 'https://api.github.com/repos/Stijnus/octotask.new-any-llm/commits/main',
  commitJson: async (branch: string) => {
    try {
      const response = await fetch(`https://api.github.com/repos/octotask/octotask/commits/${branch}`);
      const data: { sha: string } = await response.json();

      const packageJsonResp = await fetch(`https://raw.githubusercontent.com/octotask/octotask/${branch}/package.json`);
  ...
  return (
    <div className="p-4 space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium text-octotask-elements-textPrimary">Debug Information</h3>
        <div className="flex gap-2">
          <button
            onClick={handleCopyToClipboard}
            className="bg-octotask-elements-button-primary-background rounded-lg px-4 py-2 transition-colors duration-200 hover:bg-octotask-elements-button-primary-backgroundHover text-octotask-elements-button-primary-text"
          >
            Copy Debug Info
          </button>
          <button
            ...
            className={`bg-octotask-elements-button-primary-background rounded-lg px-4 py-2 transition-colors duration-200
              ${!isCheckingUpdate ? 'hover:bg-octotask-elements-button-primary-backgroundHover' : 'opacity-75 cursor-not-allowed'}
      ...
      {updateMessage && (
        <div
          className={`bg-octotask-elements-surface rounded-lg p-3 ${
            updateMessage.includes('Update available') ? 'border-l-4 border-yellow-400' : ''
          }`}
        >
          <p className="text-octotask-elements-textSecondary whitespace-pre-line">{updateMessage}</p>
          {updateMessage.includes('Update available') && (
            <div className="mt-3 text-sm">
              <p className="font-medium text-octotask-elements-textPrimary">To update:</p>
              <ol className="list-decimal ml-4 mt-1 text-octotask-elements-textSecondary">
                <li>
                  Pull the latest changes:{' '}
                  <code className="bg-octotask-elements-surface-hover px-1 rounded">git pull upstream main</code>
                </li>
                <li>
                  Install any new dependencies:{' '}
                  <code className="bg-octotask-elements-surface-hover px-1 rounded">pnpm install</code>
                </li>
      ...
      <section className="space-y-4">
        <div>
          <h4 className="text-md font-medium text-octotask-elements-textPrimary mb-2">System Information</h4>
          <div className="bg-octotask-elements-surface rounded-lg p-4">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div>
                <p className="text-xs text-octotask-elements-textSecondary">Operating System</p>
                <p className="text-sm font-medium text-octotask-elements-textPrimary">{systemInfo.os}</p>
              </div>
              <div>
                <p className="text-xs text-octotask-elements-textSecondary">Device Type</p>
                <p className="text-sm font-medium text-octotask-elements-textPrimary">{systemInfo.deviceType}</p>
              </div>
              <div>
                <p className="text-xs text-octotask-elements-textSecondary">Browser</p>
                <p className="text-sm font-medium text-octotask-elements-textPrimary">{systemInfo.browser}</p>
              </div>
              <div>
                <p className="text-xs text-octotask-elements-textSecondary">Display</p>
                <p className="text-sm font-medium text-octotask-elements-textPrimary">
                  {systemInfo.screen} ({systemInfo.colorDepth}) @{systemInfo.pixelRatio}x
                </p>
              </div>
              <div>
                <p className="text-xs text-octotask-elements-textSecondary">Connection</p>
              ...
              <div>
                <p className="text-xs text-octotask-elements-textSecondary">Screen Resolution</p>
                <p className="text-sm font-medium text-octotask-elements-textPrimary">{systemInfo.screen}</p>
              </div>
              <div>
                <p className="text-xs text-octotask-elements-textSecondary">Language</p>
                <p className="text-sm font-medium text-octotask-elements-textPrimary">{systemInfo.language}</p>
              </div>
              <div>
                <p className="text-xs text-octotask-elements-textSecondary">Timezone</p>
                <p className="text-sm font-medium text-octotask-elements-textPrimary">{systemInfo.timezone}</p>
              </div>
              <div>
                <p className="text-xs text-octotask-elements-textSecondary">CPU Cores</p>
                <p className="text-sm font-medium text-octotask-elements-textPrimary">{systemInfo.cores}</p>
              </div>
            </div>
            <div className="mt-3 pt-3 border-t border-octotask-elements-surface-hover">
              <p className="text-xs text-octotask-elements-textSecondary">Version</p>
              <p className="text-sm font-medium text-octotask-elements-textPrimary font-mono">
                {connitJson.commit.slice(0, 7)}
                <span className="ml-2 text-xs text-octotask-elements-textSecondary">
        ...
        <div>
          <h4 className="text-md font-medium text-octotask-elements-textPrimary mb-2">Local LLM Status</h4>
          <div className="bg-octotask-elements-surface rounded-lg">
            <div className="grid grid-cols-1 divide-y">
                ...
                <div key={provider.name} className="p-3 flex flex-col space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-octotask-elements-textPrimary">{provider.name}</p>
                        {provider.url && (
                          <p className="text-xs text-octotask-elements-textSecondary truncate max-w-[300px]">
                            {provider.url}
                          </p>
                        )}
                      </div>
              ...
              {activeProviders.length === 0 && (
                <div className="p-4 text-center text-octotask-elements-textSecondary">No local LLMs configured</div>
              )}
            </div>
          </div>
        </div>
