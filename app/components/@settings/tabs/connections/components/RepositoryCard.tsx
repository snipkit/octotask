...
export function RepositoryCard({ repo, onSelect }: RepositoryCardProps) {
  ...
  const getCardStyle = () => {
    return 'from-octotask-elements-background-depth-1 to-octotask-elements-background-depth-1 dark:from-octotask-elements-background-depth-2-dark dark:to-octotask-elements-background-depth-2-dark';
  };
  ...
  return (
    <motion.div
      className={`p-5 rounded-xl bg-gradient-to-br ${cardStyle} border border-octotask-elements-borderColor dark:border-octotask-elements-borderColor-dark hover:border-purple-500/40 transition-all duration-300 shadow-sm hover:shadow-md`}
      whileHover={{
        scale: 1.02,
        y: -2,
    ...
    >
      <div className="flex items-start justify-between mb-3 gap-3">
        <div className="flex items-start gap-3">
          <div className="w-10 h-10 rounded-xl bg-octotask-elements-background-depth-1/80 dark:bg-octotask-elements-background-depth-4/80 backdrop-blur-sm flex items-center justify-center text-purple-500 shadow-sm">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {repo.language && (
          <span className="flex items-center gap-1 px-2 py-1 rounded-lg bg-octotask-elements-background-depth-1/50 dark:bg-octotask-elements-background-depth-4/50 backdrop-blur-sm text-octotask-elements-textSecondary dark:text-octotask-elements-textSecondary-dark text-xs border border-octotask-elements-borderColor/30 dark:border-octotask-elements-borderColor-dark/30">
      ...
      <div className="mt-3 pt-3 border-t border-octotask-elements-borderColor/30 dark:border-octotask-elements-borderColor-dark/30 flex items-center justify-between">
        <span className="flex items-center gap-1 text-xs text-octotask-elements-textTertiary dark:text-octotask-elements-textTertiary-dark">
        ...
        {repo.topics && repo.topics.length > 0 && (
          <span className="text-xs text-octotask-elements-textTertiary dark:text-octotask-elements-textTertiary-dark">
            {repo.topics.slice(0, 1).map((topic) => (
              <span
                key={topic}
                className="px-1.5 py-0.5 rounded-full bg-octotask-elements-background-depth-1/50 dark:bg-octotask-elements-background-depth-4/50 text-xs"
              >
                {topic}
              </span>
            ))}
