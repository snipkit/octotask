...
export default function DataTab() {
  ...
  const handleExportAllChats = async () => {
    try {
      ...
      a.href = url;
      a.download = `octotask-chats-${new Date().toISOString()}.json`;
  ...
  const handleExportSettings = () => {
    try {
      const settings = {
        userProfile: localStorage.getItem('octotask_user_profile'),
        settings: localStorage.getItem('octotask_settings'),
        exportDate: new Date().toISOString(),
      };
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `octotask-settings-${new Date().toISOString()}.json`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
  ...
  const handleImportSettings = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (settings.userProfile) {
        localStorage.setItem('octotask_user_profile', settings.userProfile);
      }

      if (settings.settings) {
        localStorage.setItem('octotask_settings', settings.settings);
      }
  ...
  const handleImportAPIKeys = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      Object.entries(keys).forEach(([key, value]) => {
        ...
        localStorage.setItem(`octotask_${key.toLowerCase()}`, value);
      });
  ...
  const handleResetSettings = async () => {
    ...
    try {
      ...
      localStorage.removeItem('octotask_user_profile');
      localStorage.removeItem('octotask_settings');
      ...
      <DialogRoot open={showResetInlineConfirm} onOpenChange={setShowResetInlineConfirm}>
        <Dialog showCloseButton={false} className="z-[1000]">
          <div className="p-6">
            ...
            <p className="text-sm text-octotask-elements-textSecondary mt-2">
